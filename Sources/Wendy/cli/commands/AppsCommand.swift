    ...
    struct Remove: AsyncParsableCommand {
        ...
        func run() async throws {
            try await withAgentClientAndHostname(
                ...
                if purgeImage {
                    Noora(theme: .emerald()).success("Removed app \(appName) and its image on \(hostname)")
                } else {
                    Noora(theme: .emerald()).success("Removed app \(appName) on \(hostname)")
                }
            }
        }
    }
    ...
    struct Stop: AsyncParsableCommand {
        ...
        func run() async throws {
            try await withAgentClientAndHostname(
                ...
                Noora(theme: .emerald()).success("Stopped app \(appName) on \(hostname)")
            }
        }
    }
